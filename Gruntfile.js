// Generated on 2015-02-11 using generator-chromeapp 0.2.14
'use strict';
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
      ...
      dist: {
        options: {
          archive: function() {
            ...
            return 'package/Qd-' + manifest.version + '.zip';
          }
        },
        files: [{
          expand: true,
    ...
    sass: {
      ...
      dist: {
        files: {
          '<%= config.app %>/styles/styles.css': '<%= config.dev %>/styles/core.scss'
        }
      }
    },

    // Compile Jade from /dev to /app
    jade: {
      html: {
        files: {
          '<%= config.app %>/index.html': ['<%= config.dev %>/index.jade'],
          '<%= config.app %>/elements/elements.html': ['<%= config.dev %>/elements/elements.jade']
        },
        options: {
          pretty: true,
          data: function() {
    ...
    concat: {
      options: {
        ...
        process: function(src, filepath) {
          return '\r\r\n// Source: ' + filepath + '\n' +
          src.replace(/((^|\n)[ \t]*('use strict'|"use strict");?\s)|(var qd = qd \|\| {});?\s*/g,'$1');
        }
      },
      dist: {
        src: ['<%= config.dev %>/scripts/qd.js', '<%= config.dev %>/**/*.*.js'],
